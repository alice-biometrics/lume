...
class Setuper:
    def __init__(self, config):
        ...
        if self.setup_config:
            self.dependencies_path = os.path.join(
                config["general"]["project_path"], DEPS_FOLDER
            )
    ...
    def run(self):
        ...
        for key, setup_item_config in iteritems(self.setup_config):
            setup_stage = self.__create_setup_item(
                setup_item_config, self.dependencies_path
            )
            setup_stage.run()
